...
parser.add_argument('--model_path', type=str, default='model_pruned.pth', help='Path to pruned model')
...
def visualize(input_tensor, pred_mask, target_mask, class_colors, iou_pos, model_name, acc, out_path):
    ...
    for ax in axs:
        ax.axis('off')
    title = f"Model: {model_name}\nAcc: {acc:.4f} | Class-1 IoU: {iou_pos:.4f}"
...
def evaluate():
    ...
    if best_input is not None:
        visualize(
            ...
            os.path.basename(args.model_path), best_acc,
            out_path="best_pruned_model_inference.png"
        )
        print(f"🖼️ Saved best image with Class-1 IoU: {best_iou:.4f}")
    else:
    ...
    if valid_iou_count > 0:
        print(f"Avg Class-1 IoU: {total_iou / valid_iou_count:.4f}")
    else:
