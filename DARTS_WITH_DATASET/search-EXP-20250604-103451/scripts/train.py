...
def visualize_and_save(input_tensor, pred_mask, target_mask, save_path="segmentation_result.png"):
    """
    Visualize input image, predicted mask, and target mask side by side and save the figure.
    Assumes input_tensor shape: [C, H, W], pred_mask and target_mask shape: [H, W]
    """
    img = input_tensor.cpu().numpy()
    img = np.transpose(img, (1, 2, 0))  # C,H,W -> H,W,C
    img = np.clip(img, 0, 1)

    # Choose a colormap with enough classes, adjust NUM_CLASSES if needed
    ...
    axes[1].imshow(pred_mask.cpu(), cmap=cmap, vmin=0, vmax=NUM_CLASSES-1)
    axes[1].set_title("Predicted Mask")
    axes[1].axis('off')

    axes[2].imshow(target_mask.cpu(), cmap=cmap, vmin=0, vmax=NUM_CLASSES-1)
...
def visual_test(model, dataloader, device):
    model.eval()
    with torch.no_grad():
        for inputs, targets in dataloader:
            ...
            logits, _ = model(inputs)  # model should return logits (B, num_classes, H, W)
            preds = torch.argmax(logits, dim=1)  # (B, H, W)

            # Visualize first sample in batch
            visualize_and_save(inputs[0], preds[0], targets[0])
...
# model = ... (load your model and weights)
# test_loader = ... (your DataLoader)

# visual_test(model, test_loader, device)
