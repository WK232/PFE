...
def train(train_queue, model, criterion, optimizer):
    ...
    for step, (input, target) in enumerate(train_queue):
        ...
        if step % args.report_freq == 0:
            logging.info('Train Step: %03d Objs: %e Acc: %f Recall: %f f1: %f', step, objs.avg, top1.avg,recall_meter.avg, f1_meter.avg)
...
def infer(valid_queue, model, criterion):
    ...
    for step, (input, target) in enumerate(valid_queue):
        ...
        f1_meter.update(f1_value.item(), n)
        if step % args.report_freq == 0:
            logging.info('Valid Step: %03d Objs: %e Acc: %f Recall: %f F1: %f', step, objs.avg, top1.avg, recall_meter.avg, f1_meter.avg)
